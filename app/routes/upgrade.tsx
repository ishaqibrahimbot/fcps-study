...
export default function UpgradePage({
  ...
  // Check for both submitting and loading states
  const isSubmitting = navigation.state !== "idle";

  // Get the package being submitted (if any)
  const submittingPackage = navigation.formData?.get("package") ?? null;
      ...
      <main className="max-w-4xl mx-auto px-4 py-12">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-slate-600 dark:text-slate-400 max-w-xl mx-auto">
            Each credit unlocks one paper permanently. Choose a package that
            fits your study needs.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
          {CREDIT_PACKAGES.map((pkg) => {
            const isThisSubmitting = submittingPackage === pkg.id;
            return (
              <form key={pkg.id} method="post" className="flex">
                ...
                <button
                  ...
                  className={`flex-1 text-left rounded-2xl p-6 border-2 transition-all duration-200 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed relative ${
                    pkg.best
                ...
                >
                  {/* Loading overlay for the clicked button */}
                  {isThisSubmitting && (
